...
**silicon-based agents** (e.g. Claude Code, Gemini CLI, LLM-based assistants,
code generators, refactoring tools) may interact with this repository.
...
- ADR-001 (Ontology — place classes in correct categories)
- ADR-002 (Topology — respect layer rules)
- ADR-003 (Authority of Declarations — no inference, fail loud)
- ADR-005 (Testing as Critical Infrastructure)
- ADR-006 (Intent Contracts — update CICs when changing class behavior)
...
In this repository, silicon-based agents must additionally:

- **Respect sniffer contracts:** Do not weaken, bypass, or remove checks in `CoreConfigSniffer`, `CoreDataSniffer`, or `CorePredictionSniffer`.
- **Respect PredictionFrame invariants:** Do not make `n_rows` or `sample_count` checks optional.
- **Extend constants, not inline checks:** New supported values go in module-level constants (e.g., `SUPPORTED_LEVELS`, `SUPPORTED_DEPLOYMENT_STATUSES`), not inline string comparisons.
- **Respect the Strangler Fig:** Do not remove legacy DataFrame code paths until the PredictionFrame migration (ADR-042) is complete.
- **Do not modify `PipelineException` alerting:** The auto-WandB-alert on exception creation is intentional and load-bearing.
