# ADR-033: PredictionFrame Adoption — Strangler Fig Migration from pd.DataFrame
...
1. **Explicit config declaration.** A mandatory `prediction_format` key is added to
   ...
   no inference. This is a direct application of ADR-031.
...
The `prediction_format` config key follows directly from ADR-031: the orchestrator
