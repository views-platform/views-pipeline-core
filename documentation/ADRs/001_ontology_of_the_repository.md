...
| **Aggregation** | Ensemble prediction pooling and hierarchical reconciliation | `AggregationManager`, `ReconciliationModule`, `ForecastReconciler` | `modules/dataloaders/`; reconciliation and statistics extracted to `views-reporting` (ADR-054). Re-export shims remain in `modules/reconciliation/`, `modules/statistics/` | Stable |
| **Analysis** | Statistical computation (MAP, HDI, posterior distributions) | `PosteriorDistributionAnalyzer` | Extracted to `views-reporting` (ADR-054). Re-export shim in `modules/statistics/` | Stable |
| **Transformations** | Data transformations with undo capability | `DatasetTransformationModule` | Extracted to `views-reporting` (ADR-054); re-export shim removed 2026-07-24 (#183) after upstream deleted the module (their #119) | Retired here |
| **Reporting** | HTML report generation, geographic mapping, visualization | `ReportModule`, `MappingModule`, `PlotDistribution`, `HistoricalLineGraph`, `EvaluationReportTemplate`, `ForecastReportTemplate` | Extracted to `views-reporting` (ADR-054). Re-export shims in `modules/reports/`, `modules/mapping/`, `modules/visualizations/`, `templates/reports/` | Stable |
| **views-reporting (external)** | Visualization, statistics, mapping, reporting, reconciliation, transformations, templates | All classes from Analysis, Transformations, Reporting categories above | `views-reporting` package (sibling repo) | Evolving |
