...
The investigation identified 12 source files totaling 6,943 LOC that can be extracted as complete units, plus ~1,342 LOC of methods inside `handlers.py` that belong in outer layers (grand total: ~8,285 LOC). One module (`DatasetTransformationModule`, 1,431 LOC) has zero internal consumers. The extraction would remove 8 heavyweight dependencies from pyproject.toml (~2.5 GB install footprint reduction), plus 1 dead dependency (`properscoring`).
