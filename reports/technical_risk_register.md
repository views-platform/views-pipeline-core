...
| C-05 | 3 | **Class-level mutable state in ModelPathManager (DOWNGRADED from Tier 1).** `model.py:89` declares `_root = None` at class level, set once via lazy init, never reset. Theoretical risk: two instances from different project roots share poisoned paths. In practice: every production run, ensemble subprocess, and CI job operates from a single project root per process. No observed or reproducible scenario where root poisoning occurs. Remains a design smell that complicates testing and reasoning. | Hypothetical multi-project tool or test that creates instances from different roots without process isolation | C-05 investigation | Open |
...
| C-18 | 3 | **ReconciliationModule test coverage now substantial (DOWNGRADED from Tier 2).** 10 tests total: 6 worker characterization + 4 parallel orchestration tests (result collection, partial failure, max_workers computation, completion alert). Remaining gap: actual multiprocessing integration (tests mock ProcessPoolExecutor). | Subtle concurrency bug not caught by mocked executor; or device-specific behavior difference | test-review | Open |
