...
**Entry count:** 48 concerns + 8 disagreements
...
| C-41 | 3 | **Missing ADRs for 6 external integration decisions.** No ADR documents: (1) Appwrite storage integration — why Appwrite, contract, failure policy; (2) Three-destination persistence model — why local + views-forecasts + Appwrite, which is authoritative, partial failure semantics; (3) Graceful degradation for Appwrite — critical failure semantic undocumented; (4) PredictionSaver protocol — why Protocol pattern, format selection criteria, Track A/B rationale; (5) Rolling-origin evaluation mechanics — base_origin formula, step mapping, 13-sequence structure; (6) WandB cross-cutting integration — lifecycle management, alert routing, failure propagation. New contributors cannot distinguish intentional design from accidental structure. | New contributor makes integration change without understanding implicit contracts; or conflicting implementations added in parallel | Clean Architecture audit (2026-04-08) | Open |
