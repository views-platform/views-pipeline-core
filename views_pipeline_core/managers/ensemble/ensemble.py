...
class EnsembleManager(ForecastingModelManager):
    ...
    def _load_or_generate_prediction(
        ...
        if self._use_prediction_store:
            try:
                # New path: read via ViewsDataset.from_predstore_latest,
                # then re-materialise as the DataFrame the rest of this
                # manager expects. The dataset downloads the parquet bytes
                # from Azure (no pandas on the read path); pd.read_parquet
                # is only used here because EnsembleManager's downstream
                # code is DataFrame-coupled (the G5–G7 retirement target
                # tracked as #313/#307).
                from views_pipeline_core.modules.dataset import ViewsDataset
                dataset = ViewsDataset.from_predstore_latest(
                ...
                logger.info(
                    ...
                    f"(via ViewsDataset.from_predstore_latest)"
                )
                # Write the downloaded parquet to a local file then read it
                # back as a DataFrame. The dataset has already materialised
                # the bytes; ``save_parquet`` is a no-op disk write.
                import tempfile
                with tempfile.NamedTemporaryFile(
                    suffix=".parquet", delete=False,
                ) as tmp:
                    tmp_path = Path(tmp.name)
                dataset.save_parquet(tmp_path)
                pred = pd.read_parquet(tmp_path)
                tmp_path.unlink(missing_ok=True)
                return pred
            except (ImportError, KeyError, IndexError, ValueError, AttributeError, OSError, LookupError) as e:
                logger.info(
