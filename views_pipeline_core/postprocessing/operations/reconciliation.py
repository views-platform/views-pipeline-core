...
class ReconcilePgmWithCmPoint(PostProcessOperation):
    ...
    def process(self):
        ...
        df_to_calib[self.target] = df_to_calib[self.target].apply(
            lambda x: x[0] if isinstance(x, np.ndarray) and len(x) == 1 else x
        )

        df_calib_from[self.target] = df_calib_from[self.target].apply(
            lambda x: x[0] if isinstance(x, np.ndarray) and len(x) == 1 else x
        )
